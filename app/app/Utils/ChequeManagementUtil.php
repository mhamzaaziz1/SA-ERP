...
class ChequeManagementUtil extends Util
{
    /**
    ...
    public function getBookingsForCalendar($filters)
    {
        ...
        $query = TransactionPayment::where('business_id', $filters['business_id'])
                        ->whereBetween(DB::raw('date(cheque_due_date)'), [$filters['start_date'], $filters['end_date']]);
        ...
        foreach ($bookings as $booking) {
            ...
            $iparr = explode("2",$booking->payment_ref_no);
            if($iparr[0] == 'SP')
            {
                // $total_amount = $total_amount + $booking->amount;
                $sell_total_amount = $sell_total_amount + $booking->amount;
                $backgroundColor = '#28A76F';
                $borderColor = '#3c8dbc';
            }
             if($iparr[0] == 'PP')
            {
                $purchase_total_amount = $purchase_total_amount + $booking->amount;
            ...
            $events[] = [
                    ...
                    'start' => $booking->cheque_due_date,
                    'due_date' => 'Due date: ' . $booking->cheque_due_date,
                    'end' => $booking->cheque_due_date,
                    'payment_ref_no' => $iparr[0],
                    'amount' => $booking->amount,
                    'sell_total_amount' => $sell_total_amount,
                    'purchase_total_amount' => $purchase_total_amount,
                    'currency_id' => $business->currency_id,
                    'currency_symbol' => $currency->symbol,                    
                    'customer_name' =>'Bank name: ' . $booking->bank_name,
                    'table' => 'Cheque number: ' . $booking->cheque_number,
                    'url' => action('CheckManagementController@show', [ $booking->id ]),
                    'event_url' => action('CheckManagementController@index'),
