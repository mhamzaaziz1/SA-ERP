...
class TransactionUtil extends Util
{
            ...
            if (!empty($payment['payment_id'])) {
                ...
                if ($payment_amount != 0) {
                    ...
                    $payment_data = [
                        ...
                        'branch_code' => isset($payment['branch_code']) ? $payment['branch_code'] : null,
                        'cheque_due_date' => isset($payment['cheque_due_date']) ? $payment['cheque_due_date'] : null,
    ...
    */
    ...
    public function createExpense($request, $business_id, $user_id, $format_data = true)
    {
        ...
        // 'expense_category_id',
        // , 'contact_id'
        $transaction_data = $request->only([ 'ref_no', 'expense_reason', 'expensebeneficiary', 'transaction_date', 
            'location_id', 'expense_for', 'additional_notes', 
             'tax_id']);
    ...
    public function updateExpense($request, $id, $business_id, $format_data = true)
    {
        ...
        //     $transaction_data['sub_category_id'] = $request->input('sub_category_id');
        // }
          if ($request->has('expensebeneficiary')) {
            $transaction_data['expensebeneficiary'] = $request->input('expensebeneficiary');
        }
        ...
        // }
        if ($request->has('expense_reason')) {
            $transaction_data['expense_reason'] = $request->input('expense_reason');
        }
          $final_total = $request->final_total;
        // $request->has('final_total') ? $request->input('final_total') : $transaction->final_total;
        // $request->has('final_total')
        if ($final_total) {
            $transaction_data['final_total'] = $format_data ? $this->num_uf(
                $final_total
            ) : $final_total;
        ...
        // $transaction_data['expense_reason']   = $request->expense_reason;
    ...
    public function payContact($request, $format_data = true)
    {
        ...
        $inputs = $request->only(['amount', 'method', 'approval_number', 'approval_date', 'note', 'card_number', 'card_holder_name',
