    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
                ...
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        if (auth()->user()->can('expense.add') || auth()->user()->can('expense.edit')) {
                          ...
                          if (auth()->user()->can('expense.add') || auth()->user()->can('expense.edit')) {
                            $sub->url(
                                action('SubCategoryController@index'),
                                __('Expense SubCategories'),
                                ['icon' => 'fa fas fa-circle', 'active' => request()->segment(1) == 'Expense SubCategories']
                            );
                            // ExpenceSubCategoryController
                        }
                        if (auth()->user()->can('expense.add') || auth()->user()->can('expense.edit')) {
                            $sub->url(
                                action('ExpenseBeneficiaryController@index'),
                                __('Expense Beneficiary'),
                                ['icon' => 'fa fas fa-users', 'active' => request()->segment(1) == 'Expense Beneficiary']
                            );
                        }
                    },
            ...
            // if ( (auth()->user()->can('crud_all_bookings') || auth()->user()->can('crud_own_bookings'))) {
                $menu->url(action('CheckManagementController@index'), __('Cheque management'), ['icon' => 'fas fa fa-calendar-check', 'active' => request()->segment(1) == 'bookings'])->order(65);
            // }
