...
class Contact extends Authenticatable
{
    ...
    */
    ...
    public static function contactDropdown($business_id, $exclude_default = false, $prepend_none = true, $append_id = true)
    {
      ...
      public static function contactcreateDropdown($business_id, $exclude_default = false, $prepend_none = true, $append_id = true)
    {
        $query = Contact::where('business_id', $business_id)
                    ->where('type', '!=', 'lead')
                    ->active();
        ...
        if ($append_id) {
            // , ' - ', COALESCE(supplier_business_name, ''), '(', contact_id, ')'
            $query->select(
                DB::raw("IF(contact_id IS NULL OR contact_id='', name, CONCAT(name)) AS supplier"),
                'id'
                    );
        } else {
            $query->select(
