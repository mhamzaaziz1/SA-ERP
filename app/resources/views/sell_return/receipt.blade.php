...
<body>
        <div class="ticket">
            <div class="text-box centered">
        			        					<h3>إشعار دائن للفاتورة الضريبية المبسطة</h3>
        	</div>
            ...
            <p class="centered">
				...
				@if(!empty($receipt_details->invoice_heading))
					<br/><span class="sub-headings">@lang('lang_v1.sell_return')</span>
				@endif
			</p>
			</div>
    ...
    <table style="margin-top: 25px !important" class="border-bottom width-100 table-f-12 mb-10">
      ...
      <tbody>
		...
		@php
						$totaltax=0
						@endphp
	            				@foreach($receipt_details->lines as $key => $val)
								@php
								$totaltax=$totaltax+((float) filter_var( $val['tax'], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION ))
								@endphp
	            				@endforeach
@php
$subtotal=((float) filter_var( $receipt_details->subtotal, FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION ))-$totaltax;
$currency=' '.$subtotal.' '.substr($receipt_details->subtotal, -4);
@endphp
				...
				@if(empty($receipt_details->hide_price))
					...
					<div class="flex-box">
						...
						<p class="width-50 text-right">
							 {{$totaltax}}
							 @php
	            				echo substr($receipt_details->total,-4);
	            			@endphp
...
use Salla\ZATCA\Tags\TaxNumber;
	...
	$qrcode = GenerateQrCode::fromArray([
    			...
    			new InvoiceTotalAmount((float) filter_var( $receipt_details->total, FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION )), // invoice total amount
