...
<table class="table table-bordered ">
 <thead>
    <tr>
      <th scope="col"> <strong>{{ $transaction->business->name }}</strong></th>
      @if(!empty($transaction->location->landmark))
      <th scope="col"><b>@lang('Location'):</b></th>
      @endif
      @if(!empty($transaction->location->city) || !empty($transaction->location->state) || !empty($transaction->location->country))
      <th scope="col"><b>@lang('Country'):</b></th>
       @endif
        @if(!empty($transaction->business->tax_number_1))
      <th scope="col"><b> {{$transaction->business->tax_label_1}}:</b></th>
       @endif
        @if(!empty($transaction->business->tax_number_2))
      <th scope="col"><b>{{$transaction->business->tax_label_2}}:</b></th>
      @endif
      @if(!empty($transaction->location->mobile))
      <th scope="col"><b>@lang('contact.mobile'):</b></th>
       @endif 
       @if(!empty($transaction->location->email))
      <th scope="col"><b>@lang('business.email'):</b></th>
         @endif 
         <th scope="col"><b> @lang('Supplier'):</b></th>
    </tr>
  </thead>
  <tbody>
    <tr>
      ...
      @if(!empty($transaction->location->landmark))
      <td>{{$transaction->location->landmark}}</td>
      @endif
      @if(!empty($transaction->location->city) || !empty($transaction->location->state) || !empty($transaction->location->country))
      <td>{{implode(',', array_filter([$transaction->location->city, $transaction->location->state, $transaction->location->country]))}}</td>
       @endif
        @if(!empty($transaction->business->tax_number_1))
      <td> {{$transaction->business->tax_number_1}}</td>
       @endif
        @if(!empty($transaction->business->tax_number_2))
      <td> {{$transaction->business->tax_number_2}}</td>
      @endif 
      @if(!empty($transaction->location->mobile))
      <td>  {{$transaction->location->mobile}}</td>
       @endif 
       @if(!empty($transaction->location->email))
       <td >  {{$transaction->location->email}}
      </td>
         @endif 
          <td > {{ $transaction->contact->supplier_business_name }}
  ...
  <thead>
    <tr>
      <th scope="col"><b>@lang('Expense Requester'):</b> </th>
      <th scope="col"><b>@lang('Expense Beneficiary'):</b></th>
      <th scope="col"><b>@lang('Expence Category'):</b></th>
      <th scope="col"><b>@lang('Sub Category'):</b></th>
      <th scope="col"><b>@lang('messages.date'):</b></th>
      <th scope="col"><b>@lang('purchase.payment_status'):</b></th>
  ...
  <tbody>
    <tr>
      <td>{{ $name }}</td>
      <td>{{ $expensebeneficiary->name }}</td>
      <td>{{ $expense_categories->name }}</td>
      <td>  {{ $data->name }}</td>
